...
Feature: US1573722 - Present Retro Request to Network COO for Approval
  ...
  Scenario: TC743510 - [RL0]
    Given initiator has requested network COO approval
    When network COO is assigned a task
    Then the notification for retro approval request email is sent to the network COO as contained within the "Initiate Retro Form" (re-use the same email that was created for the ML US1562019)
    ...
    And Network COO has 3 decision options- "Approve", "Deny", "Request More Info"
  ...
  Scenario: TC785968 - [RL1]
    Given initiator has requested network COO approval
    When the Network COO selects "Deny"
